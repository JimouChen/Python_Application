...
# 传入json文件的路径
def ocr_res(json_file):
    with open(json_file, 'r', encoding='UTF-8') as f:
        global json_res
        json_res = json.load(f)

    # # 如果是网上直接传入json格式，直接下面这句,把上面的去掉
    ...
    final_msg = [handle_mark(mouth_mark_msg[0], mouth_mark_msg[1], 6),
                 handle_mark(mouth_mark_msg[2], mouth_mark_msg[3], 5),
                 handle_account(amount_msg), joiner_name[0]]
...
if __name__ == '__main__':
    ...
    # 传入json文件的路径，如果是在网上返回的json格式，稍作修改即可
    msg = ocr_res('json_rsp/res01.json')
    print(msg)
